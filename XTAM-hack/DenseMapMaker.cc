...
void DenseMapMaker::run()
{
...
#ifdef WIN32
    // Like we will never use this, just put it here in case
    ...
    while(!shouldStop()) 
    {
      ...
      // For debug to see the initialization map
      while(true);

      // Got an map, do the maintainence job blow
      // Following DTAM, we should update the costvolume if need to add new KeyFrame
