...
void reproject( const Mat& src,
    ...
    Mat cameraAffinePoseBase = cameraAffinePoseBaseIn.clone(); // Reference r
    Mat cameraAffinePoseAlternate = cameraAffinePoseAlternateIn.clone(); // One overlapping image m
    ...
    vconcat(Mat(cameraMatrix.inv()), bottom, cA_Apln); // Back-project an inverse depth value to a 3D point

    //cout<<"cA_Apln"<<cA_Apln<<endl;
    const Mat_<double>  cW_A(cameraAffinePoseAlternate.inv()); // Twa
    const Mat_<double>  cB_W(cameraAffinePoseBase); // Tbw
    Mat_<double>  cBpln_B;
    hconcat(cameraMatrix, Mat()=(Mat_<double>(3,1) << 0,0,0), cBpln_B);
    Mat_<double> cBpln_Apln = cBpln_B*cB_W*cW_A*cA_Apln; // Equation (3)
...
//    }
    ...
    warpPerspective(src, dst, cBpln_Apln, src.size(), INTER_NEAREST, BORDER_CONSTANT, Scalar(-1.0e100,-1.0e100,-1.0e100)); // Time Cost: .013 s for nearest, .021 for linear


    mask = (dst > 0); // Which pixel is visible, Time Cost: 0.00247 s 
}
